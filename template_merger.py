...
class Frame1(wx.Frame):
    ...
    def OnEditButton(self, event):
        ...
        if len(template_name) > 0:
            ...
            if platform.win32_ver()[0] != '':  # On a windows platform...
...
class TemplateMerger():
    ...
    def load_templates(self):
        ...
        for abs_template_path in glob.glob(r'%s\templates\*.template' % os.path.realpath(".")):
            local_template_path = abs_template_path.split('\\templates\\')
            if len(local_template_path) is 2:
                template_name = local_template_path[1].split('.template')
                if len(template_name) == 2:
                    templates.append(template_name[0])
        return templates
